...
for i in range(0, retries):
    try:
        ...
        if readings[2] > -20:
            print("temperature external=%f,monitor=%f" %
...
for i in range(0, retries):
    try:
        ...
        sensor_names = ["tank-sensor1", "tank-sensor2",
                        "tank-sensor3", "tank-sensor4"]
        max_temp = 85
        ...
        if len(out) > 0:
            ...
            for i in range(4):
                if (readings[i] > min_temps[i]):
                    total += (readings[i] - min_temps[i]) / \
                        (max_temp - min_temps[i])
            total = (total / 4) * 100
...
for i in range(0, retries):
    ...
    try:
        if DUMMY:
            ...
            otherReadings = list(
                map(lambda x: ctypes.c_ushort(x).value, res.registers[4:]))

        print("temperature house-downstairs=%f,house-upstairs=%f,backup=%f,rtc=%f" %
              (tempsReadings[0], tempsReadings[1], tempsReadings[2], tempsReadings[3]))
        print("heating downstairs-set-temp=%f,upstairs-set-temp=%f" %
              (otherReadings[0], otherReadings[1]))
        print("heating downstairs-state=%d,upstairs-state=%d" %
              (otherReadings[2], otherReadings[3]))
        print("heating downstairs-boost=%d,upstairs-boost=%d" %
              (otherReadings[4], otherReadings[5]))
        print("heating downstairs-off-override=%d,upstairs-off-override=%d" %
              (otherReadings[6], otherReadings[7]))
